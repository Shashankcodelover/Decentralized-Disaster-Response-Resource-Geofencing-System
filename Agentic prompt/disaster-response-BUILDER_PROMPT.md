...
Domain of this project: decentralized disaster-response coordination and geofencing
...
════════════════════════════════════════
STEP ZERO -- CHECK FOR AN INDEPENDENT REJECTION REPORT
════════════════════════════════════════

Before starting Role 1, check whether REJECTION_REPORT.md exists at
the project root.

- If it does NOT exist: proceed straight to Role 1 as normal.
- If it DOES exist: read it in full. A separate, adversarial reviewer
  agent audits this project independently and maintains this file --
  its findings are real, evidenced, and not written by you, so treat
  them as credible input, not something to argue with or explain away.
  Any OPEN item marked Critical or Major takes priority over your
  normal daily-improvement hunt for this session -- work through the
  highest-severity open items first, fixing the underlying issue
  properly (tested, traced, confirmed -- not a cosmetic patch that
  hides the symptom). Do NOT mark items "Resolved" in the file yourself
  -- that determination belongs to the Rejector's next independent
  pass. Instead, add a short note under each item you addressed
  explaining what you changed and why you believe it's fixed, so the
  Rejector has what it needs to re-verify. If you genuinely disagree an
  item is valid, don't silently ignore it -- note your reasoning next
  to it so I can see the disagreement and decide, rather than either
  side unilaterally overriding the other. Once Critical/Major open
  items are handled (or if there were none), continue into Role 1
  through Role 6 as normal. Your CHANGELOG_DAILY.md entry should note
  explicitly which rejection items you addressed today, if any.
...
of how comparable tools in decentralized disaster-response coordination and geofencing solve the same problem, what they
...
non-negotiable; any animation degrades gracefully. This tool may be used under real stress -- during or right after a disaster, by responders and possibly panicked civilians with unreliable connectivity. Every screen should assume the user is distracted, on a bad connection, and cannot afford to misread something.
...
network, concurrent use, interrupted sessions. Trace the full resource-request-to-fulfillment flow and the geofencing trigger logic end to end -- these are the core promises of the product and must be verified to actually work under bad network conditions, not just on the happy path.
...
Dependencies free of known critical vulnerabilities. Given the domain, pay special attention to location-data handling (geofencing) -- never leak precise user/victim locations beyond what's needed, and consider what happens if the system is used maliciously to track someone rather than help them.
...
Project: Decentralized-Disaster-Response-Resource-Geofencing-System
Path: d:\users\Shashank J\Desktop\my stufs\Decentralized-Disaster-Response-Resource-Geofencing-System
GitHub repo (not in use during local-only phase): https://github.com/Shashankcodelover/Decentralized-Disaster-Response-Resource-Geofencing-System.git
