...
| :--- | :--- | :--- | :--- |
| **Workspace Manager**| **Turborepo & npm**| Manages a TypeScript monorepo (`apps/server`, `apps/web`, `packages/*`) with shared compile pipelines, fast caching, and absolute package separation. | **Lerna / Yarn workspaces**: Turborepo utilizes remote caching and incremental builds, which is 10x faster. |
| **Crash Resilience** | **React Error Boundary** | Traps unexpected runtime UI/render crashes and presents a styled recovery interface with stack trace copy capabilities. | **Uncaught Crash**: Without an Error Boundary, any component render error leaves field responders with a blank white screen during a disaster. |
